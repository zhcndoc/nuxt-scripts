...
[`useScript()`{lang="ts"}](/docs/api/use-script){lang="ts"} 组合式函数是 Nuxt Scripts 的核心，用于加载所有脚本。

Nuxt Scripts 在 [`useScript()`{lang="ts"}](/docs/api/use-script){lang="ts"} 之上构建了额外的抽象层，使得以不同方式加载脚本变得更加容易。
