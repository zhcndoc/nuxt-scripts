...
对于多租户单构建部署，某个实例可以在不重新构建的情况下跳过它不使用的全局脚本。将其 `enabled` 设为 `false`，或者将 `src` 覆盖为空值，那么该脚本就不会为该部署注册：

```bash [.env per deployment]
# 仅为此实例移除此集成：
NUXT_PUBLIC_SCRIPTS_GLOBALS_AWIN_SRC=
# 或者，如果您在构建时默认值中保留了 `enabled` 字段：
...
当环境变量覆盖还不够时（例如您需要根据运行时配置计算 `src`，或者通过逻辑条件移除条目），可以使用 [`scripts:globals`](/docs/api/nuxt-app-hooks#scriptsglobals) 运行时钩子。它会在这些全局脚本注册之前，将您声明的全局脚本解析输入以可变映射的形式交给您，因此您可以在不重新构建的情况下按实例重写或 `delete` 条目，同时保留它们的 `$scripts` 类型和资源打包。
