...
- **用户 IP 匿名化** — IP 地址在转发前被匿名化到子网级别，第三方无法识别单个用户
- **减少设备指纹识别** — 屏幕分辨率、User-Agent 和硬件信息被泛化到常见类别
- **无第三方 Cookie** — 请求为同源，消除跨站追踪
- **兼容广告拦截器** — 请求看起来像第一方请求。Google 的 [server-side tagging 文档](https://developers.google.com/tag-platform/tag-manager/server-side/intro#benefits) 说明此方式提供更可靠的数据采集和更好的用户隐私控制。
- **加载更快** — 无需额外的外部域 DNS 查询

## 工作原理

启用第一方模式时：

1. **构建时**：Nuxt 下载脚本并将 URL 重写为本地路径（例如：`https://www.google-analytics.com/g/collect` → `/_scripts/c/ga/g/collect`）
2. **运行时**：Nitro 路由规则代理来自本地路径的请求到原始端点
...
每个注册的脚本根据所需数据声明默认隐私设置。隐私通过以下六个标识控制：
...
| `screen` | 将屏幕分辨率、视口、硬件并发性与设备内存泛化到通用类别 |
...
| Meta Pixel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | 不可信广告网络，完全匿名 |
| TikTok Pixel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | 不可信广告网络，完全匿名 |
| X/Twitter Pixel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | 不可信广告网络，完全匿名 |
| Snapchat Pixel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | 不可信广告网络，完全匿名 |
| Reddit Pixel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | 不可信广告网络，完全匿名 |
| Segment | - | - | - | - | - | - | 可信数据通路，要求完全数据 |
| PostHog | - | - | - | - | - | - | 可信、开源，要求完全数据 |
...
export default defineNuxtConfig({
  scripts: {
    firstParty: {
      privacy: { ip: true }, // 仅对所有脚本匿名化 IP，其他使用默认设置
    }
  }
})
```
...
::callout{type="info"}
启用某个标识时，数据会被**泛化**（降低精度）或**清空**（脱敏）——分析端点仍接收有效数据。例如屏幕分辨率 `1440x900` 会变为 `1920x1080`（桌面类别），User-Agent 会归一化为 `Mozilla/5.0 (compatible; Chrome/131.0)`{lang="ts"}，而 canvas、WebGL、插件、字体等硬件指纹会被清空或重置。
::
...
静态部署依然可以启用第一方模式，脚本会包含已重写的 URL，但您需要手动在托管平台配置重写规则。
...
第一方模式**默认启用**。对于静态托管（GitHub Pages 等），Nuxt 仍然打包脚本，但需要您配置平台重写规则让代理端点生效。
...
1. **验证代理路由是否生效** — 开发模式下检查 `/_scripts/status.json` 或 DevTools 脚本面板
2. **查看浏览器网络面板** — 确认对 `/_scripts/c/` 路径的请求存在且状态码为 200
...
1. 确保在 `nuxt.config.ts` 中正确配置了第一方路由规则
2. 静态托管环境确认重写规则已正确生效
3. 检查重写目标 URL 是否完全匹配
...
这是预期表现——静态构建无法代理请求。请配置托管平台的重写规则（见“平台重写配置”章节），或使用服务端渲染。
...
**支持自动更新。** 脚本默认缓存 7 天。缓存过期后，自动重新下载并重写 URL。您可自定义缓存时间：
...
**可以。** 通过 `collectPrefix` 自定义默认收集端点路径：
...
第一方模式主要针对客户端脚本。服务器端追踪（如 Measurement Protocol）建议直接从服务器端发送请求，无需代理。

### 如何调试被代理的请求？

1. **开发者工具**：打开 Nuxt DevTools → Scripts → 第一方标签页
...
**影响极小。** 代理带来约 10–50 毫秒的延迟，但通常被以下优势抵消：
...
ISR 页面同样支持第一方模式。打包脚本作为静态资源提供，收集请求由 Nitro 运行时代理。

### Edge 渲染

支持部署到边缘平台（如 Cloudflare Workers、Vercel Edge）。代理请求通过 Nitro 内置代理功能支持多种环境。

::callout{type="info"}
部署到边缘时，请确保运行时允许跨域请求分析端点。
::
...
- **第一方模式** 控制请求*发送路径*（通过您的服务器代理）
- **同意触发器** 控制脚本*加载时机*（同意前不注入脚本）

### 联合使用示例

```vue
<script setup>
// 通过 nuxt.config.ts 全局启用第一方模式
// 使用同意触发器控制脚本加载
...
此流程保证 GDPR 合规（先同意后追踪）且增强隐私保护（同意后第一方路由）。
...
打开 Nuxt DevTools → Scripts → 第一方标签页，查看是否启用第一方模式，配置脚本及活跃代理路由。
