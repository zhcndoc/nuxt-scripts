...
反向代理需要**服务器运行时**。完全静态的部署会提供 [`nuxt generate`](https://nuxt.com/docs/getting-started/prerendering) 的输出，但没有 Nitro 进程来处理 `/_scripts/p/**`。

Nuxt Scripts 会检测静态输出（`nuxt generate`、`nuxt build --prerender` 或静态 Nitro 预设），并自动禁用代理，同时通过构建警告列出受影响的脚本。脚本仍会被捆绑并从你的域名加载，但其采集请求会保留原始的第三方 URL，并直接发送到其源站。代理隐私和匿名化不适用于这些直接请求。

若要让请求继续通过代理并进行匿名化，请将 `nuxt build` 的服务器输出部署到运行 Nitro 的主机。
...
| 问题 | 解决方案 |
|---------|-----|
| 分析未进行跟踪 | 检查 DevTools → Network 中是否存在 `/_scripts/p/` 请求。检查 Nitro 服务器日志中的代理错误 |
| 静态网站上的代理无法正常工作 | 静态输出会自动禁用代理，采集请求会直接发送。部署 `nuxt build` 服务器输出以启用代理。请参阅[静态托管](#static-hosting-ssg) |
| 脚本过期 | 删除 `node_modules/.cache/nuxt/scripts` 并重新构建 |
| 构建下载失败 | 设置 `assets.fallbackOnSrcOnBundleFail: true`{lang="ts"} 以回退到直接加载 |
| 调试 | 打开 Nuxt DevTools → Scripts，查看代理路由和隐私状态 |
| 地理位置准确度降低 | IP 匿名化使用 IPv4 /24 或 IPv6 /48 前缀。按脚本或全局设置 `privacy: false` 以转发准确 IP |
| `bundle: false` 停止了采集代理 | 对代理支持依赖 AST 重写的脚本保持打包启用。PostHog 等自动注入的集成不需要打包；主线程 Matomo 仍会直接连接 |
| 按脚本选择退出不起作用 | 对具有自动注入功能的脚本（Plausible、PostHog、Umami、Rybbit、Databuddy），在注册表配置中使用 `proxy: false` |
