...
这是一个 Promise，当超时完成且应加载脚本时解析为 `true`，如果触发器被取消则解析为 `false`。
...
- **使用合适的超时时间**：分析脚本建议 3-5 秒，窗口小部件建议 5-10 秒，对非必要功能则设更长时间
- **考虑用户行为**：对高互动页面使用较短超时，对内容为主的页面使用较长超时
- **监控性能**：确保延迟加载确实改善了核心网页指标（Core Web Vitals）
- **结合其他触发器使用**：与交互触发器配合使用以获得更佳用户体验
