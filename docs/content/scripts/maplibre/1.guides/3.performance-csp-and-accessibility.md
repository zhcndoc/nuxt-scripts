...
Nuxt Scripts 会在 SDK 开始加载时从 `maplibre-gl` package 加载样式表。若要改为通过 Nuxt 控制样式表：
...
MapLibre v6 会从由你自己的构建提供的真实 URL 加载其 worker，因此不再需要 `worker-src blob:`。地图仍需要 `img-src data: blob:`，以及样式所使用的来源。

若要从不同路径提供 worker，请设置 `worker-url`：
