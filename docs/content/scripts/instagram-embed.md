...
Nuxt Scripts 提供了一个 [`<ScriptInstagramEmbed>`{lang="html"}](/scripts/instagram-embed){lang="html"} 组件，它在服务器端获取 Instagram 的嵌入 HTML，并通过你的服务器代理所有资源 —— 无需客户端调用 Instagram API。
